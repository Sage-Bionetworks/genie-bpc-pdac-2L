...
We'll show the table first, and detailed explanations follow below.  Each step in the table is cumulative, meaning that the second row states the number who meet the conditions in the first and second rows.  The last row is only used for the sensitivity analysis.  In the primary analysis, we assume everyone has progressed because they had a second line therapy (meaning they moved on from the first line).
...
flow_track_ft <- flow_track %>%
  ...
  fontsize(size = 10) %>%
  autofit() %>%
  italic(i = nrow(flow_track), j = 1, italic = T) 
```
